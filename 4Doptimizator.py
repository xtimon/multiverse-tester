...
try:
    ...
    class UniverseParameters:
        def __init__(self, name="Test", alpha=None, m_p=None, m_e=None, G=None):
            self.name = name
        ...
        def __repr__(self):
            return f"{self.name}: α={self.alpha:.6f}, G/G₀={self.G/self.const.G:.2f}"
    ...
    class UniverseAnalyzer:
        ...
        def calculate_habitability_index(self):
            """Усовершенствованная модель с гравитацией"""
            score = 0.0
            metrics = {}
            
            # 1. Атомная стабильность (зависит от α и m_e)
            a0_ratio = (self.u.alpha / 0.007297) * (self.u.m_e / 9.11e-31)**0.5
            if 0.3 < a0_ratio < 3:
                atomic_score = min(1.0, 1.0 - abs(a0_ratio - 1) * 0.5)
            else:
                atomic_score = 0.01  # минимальное ненулевое значение
            metrics['atomic'] = atomic_score
            score += 0.15 * atomic_score
            
            # 2. Химия (зависит от α)
            if 0.003 < self.u.alpha < 0.02:
                chem_score = 1.0 - abs(self.u.alpha - 0.007297) / 0.01
                chem_score = max(0, min(1, chem_score))
            else:
                chem_score = 0.0
            metrics['chemistry'] = chem_score
            score += 0.20 * chem_score
            
            # 3. Ядерная стабильность (зависит от α и m_p)
            binding = 8.5 * (self.u.alpha / 0.007297)**(-0.5) * (self.u.m_p / 1.67e-27)
            if 4 < binding < 12:
                nuclear_score = 1.0 - abs(binding - 8.5) / 4
            else:
                nuclear_score = 0.0
            metrics['nuclear'] = nuclear_score
            score += 0.15 * nuclear_score
            
            # 4. Звездный синтез (зависит от α, m_p, G)
            triple_alpha = math.exp(-abs(self.u.alpha - 0.007297)/0.005)
            
            # Гравитация влияет на формирование звезд
            G_ratio = self.u.G / 6.674e-11
            if 0.1 < G_ratio < 10:
                grav_score = 1.0 - abs(math.log10(G_ratio)) * 0.3
            else:
                grav_score = 0.0
            
            stellar_score = 0.7 * triple_alpha + 0.3 * grav_score
            metrics['stellar'] = stellar_score
            score += 0.25 * stellar_score
            
            # 5. Структура вселенной (зависит от G)
            # Слишком сильная гравитация → быстрое схлопывание
            # Слишком слабая → нет звезд
            if 0.3 < G_ratio < 3:
                structure_score = 1.0 - abs(G_ratio - 1) * 0.5
            else:
                structure_score = 0.0
